...
class RM_category_m extends Model
{
    ...
    {
        $db = db_connect('default');
        $builder = $this->db->table($this->table)->where('rm_category_id', $data['rm_category_id']);
        if ($builder->update($data)) {
            return $data['rm_category_id'];
        }
    }
    ...
    public function get_parent_rows_by_id($id)
    {
        $result = $this->db->table($this->table)->where('rm_category_parent_id', $id)->get()->getResult();
        return $result;
    }
    public function get_all_by_id($id)
    {
        $categories = $this->db->table($this->table)->where('rm_category_id', $id)->get()->getResult();
        return $categories;
    }
    ...
    public function get_all_specific_column()
    {
        $db = db_connect('default');
        $categories = $db->table($this->table);
        $categories->select('rm_category_id, rm_category_chain_parent');
    ...
    {
        $builder = $this->db->table($this->table);
        $builder->where('rm_category_id', $cat_id);
        $builder->delete();
    }
    // --------------------RMC status Updated-------------------------
    public function update_list_of_rm_category_status($rm_category_status = "", $rm_category_id = "")
    {
        $builder = $this->db->table("rm_category");
        $builder->set('rm_category_status', $rm_category_status);
        $builder->where('rm_category_id', $rm_category_id);
        $builder->update();
    }
    // -----------------------RMC status Updated----------------------
